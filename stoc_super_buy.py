...
# --- Strategy config ---
MAX_RISK_POINTS = 15          # skip if entry - SL > 60
...
class NiftyStrategy:
    ...
    def on_candle(self, symbol, candle):
        try:
            ...
            if pos:
                side = pos["side"]
                target = pos["target"]
                if (side == 1 and candle["high"] >= target) or (side == -1 and candle["low"] <= target):
            ...
            if is_green and sto_k > sto_d and sto_k < 40:
                ...
                # Place market buy then SL-M sell
                buy_resp = self.fyers.place_market_buy(symbol, self.traded_qty, tag="ENTRY_LONG")
                sl_resp = self.fyers.place_stoploss_sell(symbol, self.traded_qty, sl, tag="SL_LONG")
    ...
    def _attempt_target_close(self, symbol, pos, reason="tick_target"):
        ...
        try:
            ...
            # Place market close: sell for long, buy for short
            tag = "TARGET_EXIT"
            if pos["side"] == 1:
                self.fyers.place_market_sell(symbol, self.traded_qty, tag=tag)
            else:
    ...
    def on_tick(self, symbol, ltp, ts):
        ...
        try:
            ...
            if (pos["side"] == 1 and ltp >= pos["target"]) or (pos["side"] == -1 and ltp <= pos["target"]):
                if not pos.get("exit_initiated"):
    ...
    def on_trade(self, msg):
        ...
        try:
            ...
            # SL hit for long (sell trigger)
            if int(side) == -1 and "SL_LONG" in tag:
                exit_price = round_to_tick(fill_price) if fill_price else round_to_tick(sl)
                pnl = (exit_price - entry) * self.traded_qty
                log_trade(symbol, "LONG", entry, sl, target, exit_price, pnl)
            ...
            # Target exit for long (sell)
            if pos_side == 1 and int(side) == -1 and "TARGET_EXIT" in tag:
                exit_price = round_to_tick(fill_price) if fill_price else round_to_tick(target)
                pnl = (exit_price - entry) * self.traded_qty
                log_trade(symbol, "LONG", entry, sl, target, exit_price, pnl)
                logger.info(f"TARGET EXIT LONG {symbol} @ {exit_price} PnL={pnl} (tag={tag})")
                del self.positions[symbol]
                return
            ...
            if pos_side == 1 and int(side) == 1 and "ENTRY_LONG" in tag and fill_price:
                old_entry = entry
                new_entry = round_to_tick(fill_price)
                pos["entry"] = new_entry
                logger.info(f"Updated long entry price for {symbol} from {old_entry} to actual fill {new_entry}")
                return

            if pos_side == -1 and int(side) == -1 and "ENTRY_SHORT" in tag and fill_price:
                old_entry = entry
                new_entry = round_to_tick(fill_price)
                pos["entry"] = new_entry
