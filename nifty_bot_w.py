...
LOT_SIZE = 20  # official NIFTY option lot size
...
class NiftyBuyStrategy:
    def __init__(self, fyers_client: FyersClient, lot_size: int = 20):
        ...
        self.pending_orders = {}  # Orders placed but not filled
    ...
    def on_candle(self, symbol, candle):
        ...
        # only place entry if no position and no pending order
            ...
            if (not REQUIRE_W_CONTEXT) or (w_phase == 3):
                if candle["close"] > vwma20 and candle["close"] > ema5:
                    ...
                    self.pending_orders[symbol] = {
                        ...
                        "target": target_price, "resp": resp
                    }
                    logging.info(f"ENTRY ORDER PLACED {symbol} @ {entry_price}")

        # active position target exit
    ...
    def on_trade(self, msg):
        ...
        if side == 1 and tag == "NIFTYBUYENTRY":
            ...
            if pending:
                ...
                logging.info(f"ENTRY FILLED {symbol}, SL placed @ {sl_price}")
...
if __name__ == "__main__":
    ...
    option_symbols = ["BSE:SENSEX2590481000CE","BSE:SENSEX2590481000PE"]  # replace with actual symbol
