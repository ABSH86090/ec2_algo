...
Entry Signal (evaluated independently for CE and PE on each closed candle):
  1. Candle close > EMA5
  2. EMA5 < EMA5_TO_EMA20_RATIO × EMA20
...
EMA5_TO_EMA20_RATIO   = 0.90   # EMA5 must be below this fraction of EMA20
...
class OptionSideEngine:
    ...
    def on_closed_candle(self, candle):
        ...
        logger.info(
            ...
            f"C={candle['close']:.2f} EMA5={ema5:.2f} EMA20={ema20:.2f} "
        ...
        if (
            ...
            logger.info(
                f"[{self.side} SIGNAL] close={candle['close']:.2f} > EMA5={ema5:.2f} | "
                f"EMA5/EMA20={ema5/ema20*100:.1f}% < {EMA5_TO_EMA20_RATIO*100:.0f}% "
                f"→ pending entry at next candle open"
            )
            send_telegram(
                f"⚡ {self.side} SIGNAL\n"
                f"Close={candle['close']:.2f} > EMA5={ema5:.2f}\n"
                f"EMA5/EMA20 = {ema5/ema20*100:.1f}% (threshold {EMA5_TO_EMA20_RATIO*100:.0f}%)\n"
