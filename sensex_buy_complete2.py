...
# Trigger conditions (kept for reference but NOT used in entry now)
...
# ADX filter (computed for logging; not used in entry logic now)
ADX_PERIOD = 14
ADX_MIN = 15
...
class NiftyBuyStrategy:
    def __init__(self, fyers_client: FyersClient, lot_size: int = 20):
        ...
        # state: W span + delayed entry sequence
        self.state = defaultdict(lambda: {
            "w_span": None,
            "last_trade_idx": -1,
            "breakout_time": None,      # time of base green candle above EMA5/VWMA20 (for logging, not cutoff)
            "trigger_base_idx": None,   # index of that base green candle
            "seen_red_after_trigger": False  # whether we've seen a red candle after base
        })
    ...
    def adx_series(self, highs, lows, closes, period=14):
        ...
        # ADX: average of DX values using Wilder smoothing. First ADX at index = 2*period
    ...
    # --- NEW: upper-wick helpers (kept; not used in entry now) ---
    ...
    def _extract_order_id(self, order_resp):
        ...
        try:
            ...
            if isinstance(order_resp, dict):
                ...
                if (order_resp.get("orders") and isinstance(order_resp["orders"], dict)
                        and order_resp["orders"].get("id")):
        ...
        try:
            ...
            if curr_ltp is not None:
                ...
                if new_sl >= max_allowed_sl:
                    logger.info(
                        f"Trailing SL not safe to place for {symbol}: desired {new_sl} "
                        f">= max_allowed {max_allowed_sl} (ltp={curr_ltp})"
                    )
                    return None, None
        ...
        try:
            ...
            try:
                snapshot = {s: {
                    ...
                    'EMA5': _safe(self.ema_series(
                        [c['close'] for c in self.candles[s]],
                        EMA_PERIOD
                    )[-1]) if len(self.candles[s]) > 0 else None,
                    'VWMA20': _safe(self.vwma_series(
                        [c['close'] for c in self.candles[s]],
                        [c['volume'] for c in self.candles[s]],
                        VWMA_PERIOD
                    )[-1]) if len(self.candles[s]) > 0 else None,
            ...
            if ema5 is None or vwma20 is None:
                logger.debug(
                    f"Not enough data for indicators for {symbol} at idx={idx} "
                    f"(EMA5={ema5}, VWMA20={vwma20}, ADX={adx})"
                )
                return
            ...
            logger.info(json.dumps({
                ...
                'EMA5': _safe(ema5),
                'VWMA20': _safe(vwma20),
                'ADX': _safe(adx),  # informational only
                'w_span': st.get('w_span'),
                'in_position': bool(pos),
                'trigger_base_idx': st.get('trigger_base_idx'),
                'seen_red_after_trigger': st.get('seen_red_after_trigger')
            }))
            ...
            if idx <= low2_idx:
                # still within the W formation
                return

            # --- Step A: identify base green breakout candle (no entry here) ---
            # Conditions:
            #   - green candle
            #   - close > EMA5 and close > VWMA20
            #   - EMA5 > VWMA20
            if st["trigger_base_idx"] is None:
                if (self.is_green(candle) and
                        candle["close"] > ema5 and
                        candle["close"] > vwma20 and
                        ema5 > vwma20):
                    st["trigger_base_idx"] = idx
                    st["seen_red_after_trigger"] = False
                    st["breakout_time"] = candle["time"]  # for logging only
                    logger.info(json.dumps({
                        'event': 'base_breakout_detected',
                        'symbol': symbol,
                        'idx': idx,
                        'time': str(candle["time"]),
                        'close': candle["close"],
                ...
                return  # wait for future candles

            # From here, we already have a base breakout identified
            base_idx = st["trigger_base_idx"]
            if idx <= base_idx:
                return  # safety guard

            # --- Step B: wait for a red candle AFTER base breakout ---
            if not st["seen_red_after_trigger"]:
                if self.is_red(candle):
                    st["seen_red_after_trigger"] = True
                    logger.info(json.dumps({
                        'event': 'red_candle_after_base',
                        'symbol': symbol,
                        'idx': idx,
                        'time': str(candle["time"]),
                        'close': candle["close"]
                    }))
                # still waiting for green entry candle
                return
            ...
            if (self.is_green(candle) and
                ...
                try:
                    ...
                    # fail-safe: allow entry if cooldown cannot be computed

                # ENTRY: at close of this *second* green candle
                entry = round_to_tick(candle["close"])

                # SL = entry green candle's low - 3 points
                sl_price = round_to_tick(candle["low"] - 3.0)
                ...
                # Target = 1R (target points = SL distance)
                target_price = round_to_tick(entry + risk_points)

                # Strike selection / premium logic (same as before)
                ...
                try:
                    if candle["close"] > 700:
                        try:
                            ...
                            logger.warning(
                                f"Failed to adjust strike for {symbol}: {e}. "
                                f"Proceeding with original symbol."
                            )
                except Exception:
                    # defensive: if candle has no 'close' or weird type, continue with original symbol
                    pass

                # Place market buy then SL-M on selected symbol
                ...
                # set last_trade_time for cooldown
                try:
                ...
                logger.info(json.dumps({
                    ...
                    'risk_points': risk_points,
                    'target': target_price,
                    'multiplier': 1.0,  # 1R: target distance equals SL distance
                    'EMA5': _safe(ema5),
                    'VWMA20': _safe(vwma20),
                    'seen_red_after_trigger': st["seen_red_after_trigger"],
                    'buy_resp': str(buy_resp),
                    'sl_resp': str(sl_resp)
                }))

                # reset W + sequence so we don’t re-enter off the same pattern
                st["w_span"] = None
    ...
    def _attempt_target_exit(self, symbol, pos, reason="tick_target"):
        ...
        if (now_ms - last_ms) < EXIT_THROTTLE_MS:
            logger.debug(
                f"Throttle active for {symbol} exit attempts "
                f"(wait {(EXIT_THROTTLE_MS - (now_ms - last_ms))} ms)."
            )
            return
        ...
        try:
            ...
            if sl_order and isinstance(sl_order, dict):
                sl_order_id = sl_order.get("id") or (sl_order.get("raw") and sl_order["raw"].get("id"))
    ...
    def on_tick(self, symbol, ltp, ts):
        ...
        try:
                ...
                if trail_step <= pos.get("last_trail_step", -1):
                    ...
                    if desired_sl >= max_allowed_sl:
                        ...
                        if clamped > pos["sl"] and clamped < ltp:
                            ...
                            logger.debug(
                                f"Trailing SL for {symbol}: desired {desired_sl} unsafe (>= {max_allowed_sl}); skipping."
                            )
                            desired_sl = None

                    if desired_sl and desired_sl > pos["sl"]:
                        try:
                            sl_resp, sl_id = self._safe_cancel_and_place_sl(
                                symbol, pos, desired_sl, tag="NIFTYSLTRAIL"
                            )
                            if sl_resp:
                                pos["sl"] = desired_sl
                                if sl_id:
                                ...
                                logger.info(
                                    f"Trailing SL applied for {symbol}: "
                                    f"step={trail_step}, sl={desired_sl}, max_ltp={pos['max_ltp']}"
                                )
                            else:
                                logger.debug(
                                    f"No new SL placed for {symbol} (desired_sl={desired_sl})"
                                )
                        except Exception as e:
            ...
            if ltp >= pos["target"] and not pos.get("exit_initiated"):
                logger.debug(
                    f"Tick-level target detected for {symbol} "
                    f"ltp={ltp} target={pos['target']}"
                )
