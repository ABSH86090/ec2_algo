...
# Reset logging handlers (important if you rerun in same session)
...
class FyersClient:
    ...
    def subscribe_market_data(self, instrument_ids, on_candle_callback):
        ...
        def on_message(tick):
            try:
                # Ignore connection/control messages
                if "symbol" not in tick or "ltp" not in tick:
                    logging.debug(f"Ignored non-tick message: {tick}")
                    return

                symbol = tick["symbol"]
                ltp = float(tick["ltp"])
                ts = int(tick["last_traded_time"])   # epoch seconds
...
class StrategyEngine:
    ...
    def on_candle(self, symbol, candle):
        ...
        # ✅ Log indicators on every new candle
        ...
        if self.detect_pattern(candles) and ema5 < vwma20 and self.position is None:
            ...
            logging.info(f"ENTRY: {symbol} @ {entry_price}, SL: {sl_price}, EMA5={ema5:.2f}, VWMA20={vwma20:.2f}")
        ...
        if self.position and candle["close"] > ema5 and candle["close"] > vwma20:
            ...
            logging.info(f"EXIT (Cond): {self.position['symbol']} EMA5={ema5:.2f}, VWMA20={vwma20:.2f}, Close={candle['close']}")
            print(f"EXIT: {self.position['symbol']} by condition")
            self.position = None

    def on_trade(self, msg):
        if self.position and msg.get("trades") and msg["trades"]["side"] == 1:
            logging.info(f"EXIT (SL): {self.position['symbol']} SL hit, Trade={msg}")
...
if __name__ == "__main__":
    ...
    # Prefill with history before sockets start
